...
public class BoardService {
    ...
    public ColumnDto createColumn(String workspaceId, ColumnCreateRequest req, User currentUser) {
        ...
        ColumnDto columnDto = ColumnDto.fromEntity(col);
        webSocketEventPublisher.publishBoardEvent(workspaceId, new BoardEvent(BoardEvent.Type.COLUMN_CREATED, columnDto));
        return columnDto;
    }
...
public ColumnDto updateColumn(String workspaceId, String columnId,
        ...
        columnRepository.save(col);
        ColumnDto columnDto = ColumnDto.fromEntity(col);
        webSocketEventPublisher.publishBoardEvent(workspaceId, new BoardEvent(BoardEvent.Type.COLUMN_UPDATED, columnDto));
        return columnDto;
    }
...
public ColumnDto patchColumn(String workspaceId, String columnId,
    ...
    public void deleteColumn(String workspaceId, String columnId, User currentUser) {
        authorizationService.checkOwnerOrAdmin(workspaceId, currentUser);
        BoardColumn column = findColumnInWorkspace(columnId, workspaceId);
        columnRepository.deleteById(columnId);
        webSocketEventPublisher.publishBoardEvent(workspaceId, new BoardEvent(BoardEvent.Type.COLUMN_DELETED, ColumnDto.fromEntity(column)));
    }
    ...
    public TaskDto createTask(String workspaceId, TaskCreateRequest req, User currentUser) {
        ...
        int newCounter = workspace.getTaskCounter() + 1;
        workspace.setTaskCounter(newCounter);
        workspaceRepository.save(workspace);
        String publicId = workspace.getKeyPrefix() + "-" + newCounter;
        Task task = Task.builder()
                ...
                .assigneeId(validateAssigneeId(req.getAssigneeId(), workspaceId))
                .dueDate(req.getDueDate())
                .build();
        taskRepository.save(task);
        TaskDto taskDto = TaskDto.fromEntity(task);
        webSocketEventPublisher.publishBoardEvent(workspaceId, new BoardEvent(BoardEvent.Type.TASK_CREATED, taskDto));
        return taskDto;
    }

    public void assignTaskToUser(String workspaceId, String taskId, String userId, User currentUser) {
        authorizationService.checkAccess(workspaceId, currentUser);
        Task task = findTaskInWorkspace(taskId, workspaceId);
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new EntityNotFoundException("User", userId));
        boolean isUserMember = authorizationService.isUserMemberOfWorkspace(workspaceId, userId);
        if (!isUserMember) {
    ...
    public void updateTaskColumn(String workspaceId, String taskId, String newColumnId, User currentUser) {
        authorizationService.checkAccess(workspaceId, currentUser);
        Task task = findTaskInWorkspace(taskId, workspaceId);
        BoardColumn column = findColumnInWorkspace(newColumnId, workspaceId);
        task.setColumnId(newColumnId);
        taskRepository.save(task);
    }

    public void addLabelToTask(String workspaceId, String taskId, String labelId, User currentUser) {
        authorizationService.checkAccess(workspaceId, currentUser);
        Task task = findTaskInWorkspace(taskId, workspaceId);
        ...
        if (task.getLabels() == null) {
            task.setLabels(new java.util.HashSet<>());
        }
        task.getLabels().add(label);
        taskRepository.save(task);
    }

    public void removeLabelFromTask(String workspaceId, String taskId, String labelId, User currentUser) {
        authorizationService.checkAccess(workspaceId, currentUser);
        Task task = findTaskInWorkspace(taskId, workspaceId);
        Label label = labelRepository.findById(labelId)
                .orElseThrow(() -> new EntityNotFoundException("Label", labelId));
...
public TaskDto updateTask(String workspaceId, String taskId,
        ...
        if (req.getAssigneeId() != null) task.setAssigneeId(validateAssigneeId(req.getAssigneeId(), workspaceId));
        if (req.getDueDate() != null) task.setDueDate(req.getDueDate());
        taskRepository.save(task);
        TaskDto taskDto = TaskDto.fromEntity(task);
        webSocketEventPublisher.publishBoardEvent(workspaceId, new BoardEvent(BoardEvent.Type.TASK_UPDATED, taskDto));
        return taskDto;
    }
    ...
    public void deleteTask(String workspaceId, String taskId, User currentUser) {
        ...
        webSocketEventPublisher.publishBoardEvent(workspaceId, new BoardEvent(BoardEvent.Type.TASK_DELETED, TaskDto.fromEntity(task)));
    }
    ...
    public LabelDto createLabel(String workspaceId, LabelDto req, User currentUser) {
        ...
        LabelDto labelDto = LabelDto.fromEntity(label);
        webSocketEventPublisher.publishBoardEvent(workspaceId, new BoardEvent(BoardEvent.Type.LABEL_CREATED, labelDto));
        return labelDto;
    }
    ...
    public void deleteLabel(String workspaceId, String labelId, User currentUser) {
        ...
        webSocketEventPublisher.publishBoardEvent(workspaceId, new BoardEvent(BoardEvent.Type.LABEL_DELETED, LabelDto.fromEntity(label)));
    }
    ...
    private String validateAssigneeId(String assigneeId, String workspaceId) {
        if (assigneeId == null) return null;
        if (!userRepository.existsById(assigneeId)) {
            throw new EntityNotFoundException("User", assigneeId);
        }
        if (!authorizationService.isUserMemberOfWorkspace(workspaceId, assigneeId)
                && !authorizationService.isOwner(workspaceId, assigneeId)) {
            throw new ForbiddenException("Assignee bu workspace a'zosi emas");
        }
        return assigneeId;
    }
}
