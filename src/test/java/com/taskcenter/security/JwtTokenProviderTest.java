...
class JwtTokenProviderTest {
    ...
    @Test
    void validateConfig_rejectsDefaultSecret() {
        JwtTokenProvider p = new JwtTokenProvider();
        ReflectionTestUtils.setField(p, "jwtSecret",
                "super-secret-key-that-needs-to-be-at-least-256-bits-long-for-hs256-algorithm-to-work-properly");
        ReflectionTestUtils.setField(p, "jwtExpirationMs", 604800000);

        org.junit.jupiter.api.Assertions.assertThrows(IllegalStateException.class, p::validateConfig);
    }

    @Test
    void validateConfig_rejectsTooShortSecret() {
        JwtTokenProvider p = new JwtTokenProvider();
        ReflectionTestUtils.setField(p, "jwtSecret", "short");
        ReflectionTestUtils.setField(p, "jwtExpirationMs", 604800000);
    ...
    @Test
    void validateConfig_acceptsValidSecret() {
        JwtTokenProvider p = new JwtTokenProvider();
